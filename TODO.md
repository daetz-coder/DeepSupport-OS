...
- [ ] 在仓库根执行 `docker compose up --build -d`，确认 `docker-compose.yml` 中 `api`（宿主机 `18000`）与 `frontend`（`5173`）起来；用 `http://127.0.0.1:18000/health` 与 `http://127.0.0.1:5173/` 做本地可达性检查
- [ ] 若 compose 失败或 Docker 不可用：按 README 本地起 `backend`（`uv run deepsupport-os`）+ `frontend`（`npm run dev`），或改跑 `scripts\demo-public.ps1 -LocalDev -SkipTunnel` 先验证本机闭环
